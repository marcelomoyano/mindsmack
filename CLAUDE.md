...
- **Serving:** because `orb.js` is an external ES module, the page can no longer be opened via `file://` (Chrome blocks local module scripts as a CORS/opaque-origin issue) — **serve it over http** (`python3 -m http.server 5500` from the project root, then `http://localhost:5500/mindsmack-landing.html`). Everything else (CSS, the inline scroll-reveal script) still works from `file://`; only the orb needs the server.
